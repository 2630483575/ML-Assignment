...
from data.dataset import load_conll2003, get_label_mappings, print_dataset_summary
...
def main():
    ...
    # Mode selection
    parser.add_argument("--model", type=str, default="crf", choices=["crf", "bilstm"], help="Model type")
    ...
    # BiLSTM arguments
    parser.add_argument("--epochs", type=int, default=20, help="BiLSTM epochs")
    parser.add_argument("--batch_size", type=int, default=32, help="BiLSTM batch size")
    parser.add_argument("--lr", type=float, default=0.001, help="BiLSTM learning rate")
    ...
    if config.model_type == "crf":
        ...
        if config.mode == "train":
            ...
            # For CRF, train and evaluate is usually done together or we load a saved model
            # Here we'll just run the train method which includes evaluation
        ...
        if config.mode == "train":
            ...
            # This assumes best_bilstm_crf.pt exists
            print("Evaluation mode for BiLSTM not fully separated yet, running training loop will evaluate at end.")
            trainer.train(dataset)
